...
?>
	<div class="l-inner">
		<div id="cb_<?php echo $key; ?>" class="p-index-slider-content01">
...
?>
		<div id="cb_<?php echo $key; ?>" class="p-index-content02">
			<div class="l-inner">
				<h2 class="business-ttl content02-ttl">
				<?php if($key == '5'): ?>
					不動産会社様の声
				<?php else: ?>
					一般法人・<br class="sp-br">フリーランスの声
				<?php endif; ?>
				<!-- <p class="sub-ttl"></p> -->
				</h2>
	...
	<div class="l-inner">
		<div id="cb_<?php echo $key; ?>" class="p-index-content04">
			...
			<div class="p-index-content04__desc<?php if ( ! $value['cb_gallery_content_desc1'] || ! ( $value['cb_gallery_content_desc2'] ) ) : ?> p-index-content04__desc--full<?php endif; ?>">
				<?php if($value['cb_gallery_content_desc1']){ ?><p style="font-size: <?php echo esc_attr( $value['cb_gallery_content_desc_font_size1'] ); ?>px;"><?php echo nl2br( esc_html( $value['cb_gallery_content_desc1'] ) ); ?></p><?php }; ?>
				<?php if($value['cb_gallery_content_desc2']){ ?><p style="font-size: <?php echo esc_attr( $value['cb_gallery_content_desc_font_size2'] ); ?>px;"><?php echo nl2br( esc_html( $value['cb_gallery_content_desc2'] ) ); ?></p><?php }; ?>
			</div>
		</div>
	</div><!-- /.l-inner -->
<?php 
...
<!-- 工事中 -->
		...
		<div id="cb_<?php echo $key; ?>" class="p-index-content06">
			<div class="l-inner">
				<h2 class="business-ttl">パートナー様への<br class="sp-br"><span class="font-w">5つのお約束</span>
				<!-- <p class="sub-ttl"></p> -->
				</h2>
		...
		elseif ( 'wysiwyg' == $value['cb_content_select'] ) : 
      	...
      	$cb_wysiwyg_editor = apply_filters( 'the_content', $value['cb_wysiwyg_editor'] );
				if ( $cb_wysiwyg_editor ) {
					echo '<div id="cb_' . $key . '" class="p-entry__body clearfix">' . $cb_wysiwyg_editor . '</div>' . "\n";
				}
			} 
?>
<?php 
